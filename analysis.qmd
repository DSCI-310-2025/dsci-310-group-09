...
ggplot(data, aes(x = class, fill = class)) +
  geom_bar() +
  theme_minimal() +
  labs(title = "Table1: Distribution of Car Evaluations", x = "Class", y = "Count")
```
...
plot_list <- lapply(features, function(feature) {
  ggplot(data, aes(x = .data[[feature]], fill = class)) +
    ...
    theme_minimal() +
    labs(title = paste("Table 2 - 7:", feature, "vs. Evaluation Class"))
})
...
ggplot(df_balanced, aes(x = class, fill = class)) +
  geom_bar() +
  theme_minimal() +
  labs(title = "Table 8: Distribution of Car Evaluations After Feature Engineering", x = "Class", y = "Count")
```
...
plot_list <- lapply(features, function(feature) {
  ggplot(data, aes(x = .data[[feature]], fill = class)) +
    geom_bar(position = "dodge") +
    theme_minimal() +
    labs(title = paste("Table 9 - 14:",feature, "vs. Evaluation Class"))
})
...
title("Table 15: Correlation Heatmap")
```
